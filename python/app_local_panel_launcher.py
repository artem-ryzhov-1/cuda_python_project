########################################
# python/app_local_panel_launcher.py
########################################
...
if platform_type == 'colab_linux':
    ...
    repo_path = Path('~/cuda_python_project').expanduser()
elif platform_type == 'local_windows':
    repo_path = Path(os.path.expandvars(r'%USERPROFILE%\cuda_python_project'))
...
# CRITICAL FIX: Wrap app creation in a function
def create_app():
...
# Make the function servable (not the app itself)
create_app().servable()